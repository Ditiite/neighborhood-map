...
export default class ApiWrapper extends Component {
  ...
  componentDidMount() {
    ...
    scriptEl.async = true;
    scriptEl.src =
      "https://maps.googleapis.com/maps/api/js?key=AIzaSyAi8HndyCibVRx205QFrZ2MVORDaGABPjE&callback=googleSuccess&libraries=places&v=3&language=en";
    scriptEl.onerror = () => {
        ...
        >
          ...
          {this.state.error && (
            <div className="error-msg">
              Something went wrong! Service not available
              <br /> Please try later!
            </div>
          )}
        </div>
        {!this.state.loading && <MapContainer google={this.state.google} />}
      </React.Fragment>
    );
  }
}
